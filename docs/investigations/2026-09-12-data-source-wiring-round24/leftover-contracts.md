...
Closed in this round (not leftovers): catalog rescan leftover-globbed TickFlow calendars after a provider switch; catalog list/get leftover serving after a daily / adj / depth / realtime switch (token now includes those routes); leftover-only financial bytes no longer become a current calendar; membership history rewrote leftover as current after a pool switch; reference query leftover-globbed valuation / membership / corporate-actions; quote snapshots reused leftover TickFlow after a realtime switch (write now tags; reads filter); valuation / limit-up writes now carry daily route; corporate-actions prior merge skipped leftover after an adj switch; daily quality leftover-globbed TickFlow enriched turnover after a daily switch. Leftover TickFlow still sees untagged partitions.
