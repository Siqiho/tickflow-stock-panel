...
结果：`1040 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round32`）。

Round-thirty-two tests cover unreadable tagged leftover no longer mixing untagged extras in instruments / financials / depth / leftover TickFlow `kline_ext`, leftover TickFlow DuckDB views skipping untagged extras beside tagged leftover, leftover TickFlow adj / minute-batch / monitor / burst jobs skipping after custom daily, unreadable user-ext date markers skipping, leftover TickFlow + free realtime `mode=none`, leftover TickFlow daily + public realtime overlay kept, untagged-only leftover still served, after-hours default clock times unchanged.
