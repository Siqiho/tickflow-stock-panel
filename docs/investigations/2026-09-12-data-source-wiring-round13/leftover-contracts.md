# Intentional leftover TickFlow contracts (round 13)
...
Closed in this round (not leftovers): HTTP extend-minute raw merge + ungated `CREATE VIEW`, minute incremental start / status counting leftover TickFlow partitions after a custom switch, DuckDB adj/minute/financial gates undone by `clear_data` or `refresh_index_views`, public financial merge writing into a custom financial file, share-history PIT append mixing stale TickFlow shares, corporate-actions `fetch_missing_adj` writing public sina factors under a custom adj route.
