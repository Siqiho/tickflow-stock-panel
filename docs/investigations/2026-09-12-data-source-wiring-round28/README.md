...
| quote_snapshot HTTP 覆盖 | 自定义日K仍叠 leftover / public 快照 | 切源后日K图混入 leftover TickFlow 蜡烛 |
...
4. **覆盖**：自定义 / unresolved daily 不再叠 quote_snapshot；默认 leftover TickFlow daily + public realtime 仍覆盖。
5. **Lab**：仅 `route=public` 可走公开 Lab；leftover TickFlow 409。
6. **untagged extras**：同日有 tagged leftover 时丢掉 untagged。

未改：盘后默认时刻（只改 job 是否拉 TickFlow 维表）、leftover TickFlow 单票公开分时 / 自选历史 TDX、leftover TickFlow + free 仍是 `mode=none`、leftover TickFlow 仍可见「只有 untagged」的旧分区与不可读日期标记、`.env`、鉴权。
...
云环境结果：`948 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round28`）。详见 `evidence/test-results.md`。
