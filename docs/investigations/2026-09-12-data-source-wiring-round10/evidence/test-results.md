...
Round-ten tests cover live-daily persist gate (custom daily → quote snapshot only; leftover TickFlow still writes canonical), `sync_daily_by_quotes` internal gate, minute cache provenance (stale TickFlow tagged/untagged rejected for custom; leftover untagged still valid), HTTP `/api/kline/minute` skip of stale local, minute write route tag, custom full_minute no `minute_data_provider` mix, leftover full_minute still uses monitor batch, public adj empty scope fail-closed, depth `_has_capability` honesty, financial cache provenance, and leftover TickFlow contracts (mode=none, undeclared daily, prefs-unreadable persist block).
