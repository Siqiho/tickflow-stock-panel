...
结果：`1111 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round35`）。

Round-thirty-five tests cover kline_loader leftover-glob no longer mixing untagged extras beside tagged leftover, leftover TickFlow financial `_sync_table` / scheduler body skipping after custom daily, leftover TickFlow QuoteService watchlist / full-market skipping after custom daily, financial HTTP availability staying fail-closed on prefs throw, leftover-part-only public financial merge / period-stats still serving extras-only leftover, ext delete / provenance / snapshot remount no longer leftover-unioning unreadable extras, leftover TickFlow + free realtime `mode=none`, leftover TickFlow daily + public realtime overlay kept, untagged-only leftover still served, after-hours default clock times unchanged. Catalog / get_minute stay fail-loud on leftover TickFlow.
