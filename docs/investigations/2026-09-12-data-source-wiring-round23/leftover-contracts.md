# Intentional leftover TickFlow contracts (round 23)
...
Closed in this round (not leftovers): DataStore init leftover-visible raw kline/adj/financial/depth globs; regime / mainline on-disk history merged leftover TickFlow after a daily switch; pool membership seeded leftover CSI after a pool switch; financial PIT migration rewrote leftover as current; catalog compatibility calendars served last-scan leftover dates after a provider switch (hot path now keys by prefs route token). Leftover TickFlow still sees untagged partitions.
