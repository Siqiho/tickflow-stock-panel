...
Round-fifteen tests cover enriched pipeline replacing stale TickFlow partitions (leftover TickFlow still publishes untagged daily), unresolved daily skipping enriched publish, public financial/adj merge refusing writes when the route gate raises, daily/enriched status hiding leftover dirs under custom (leftover TickFlow still counts untagged and unreadable date markers), adj status daily window ignoring leftover daily under custom daily, reference valuation/limit/sealed-L1 skipping leftover inputs, regime date set ignoring leftover enriched under custom, operational `latest`/`earliest` daily and enriched dates using provenance, and leftover TickFlow contracts (mode=none, undeclared daily, prefs-unreadable persist block).
