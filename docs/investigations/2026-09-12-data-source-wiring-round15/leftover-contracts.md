# Intentional leftover TickFlow contracts (round 15)
...
| Historical daily / enriched partitions until re-sync | `GET /api/kline/daily`, screener disk path | Switching daily source does not rewrite history; live overlay is snapshot-only when daily is custom. Same-date *writes* and derived/status/pipeline coverage now use provenance. |
...
Closed in this round (not leftovers): enriched pipeline concat-mix and leftover-daily rematerialize, public financial/adj merge fail-open on gate exception, daily/enriched status counting leftover dirs, adj status leftover daily window, operational `latest`/`earliest` daily and enriched dates, reference valuation/limit/sealed-L1 leftover inputs, regime date set treating leftover enriched as current.
