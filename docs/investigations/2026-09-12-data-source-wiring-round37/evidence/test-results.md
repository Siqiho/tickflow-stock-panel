...
结果：`1149 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round37`）。

Round-thirty-seven tests cover capability labels for adj / depth / quote / minute no longer advertising live TickFlow after custom daily, DepthService leftover TickFlow lifecycle / `_depth_source` skipping after custom daily, QuoteService leftover TickFlow `realtime_mode` becoming `none` after custom daily, settings route refresh stopping leftover TickFlow depth / quote loops, leftover TickFlow `_find_universe_id` skipping after custom daily, catalog instruments no longer leftover-serving after custom daily, leftover TickFlow + free realtime `mode=none`, leftover TickFlow daily + public realtime overlay kept, untagged-only leftover still served, after-hours default clock times unchanged. Catalog / get_minute stay fail-loud on leftover TickFlow.
