# Intentional leftover TickFlow contracts (round 14)
...
| Leftover TickFlow adj coverage / public sina writes | `read_adj_coverage`, `sync_adj_factor_public` | Same leftover sina contract; custom/unresolved now refuse |
...
| Lab `/api/free-ext` explicit public writes | `free_ext.py` | Lab is an explicit public write surface; custom adj/financial now fail-closed at the writer so Lab cannot silent-mix |
| Historical daily / enriched partitions until re-sync | `GET /api/kline/daily`, screener disk path | Switching daily source does not rewrite history; live overlay is snapshot-only when daily is custom. Same-date *writes* now replace other-route bars. |
...
Closed in this round (not leftovers): same-date daily/enriched concat mix on persist/live-merge, public adj coverage leaking into custom status / corporate-actions, `sync_adj_factor_public` / `merge_write_adj_*` writing sina under custom adj, public share-capital writing into a custom financial file, reference-derived shares raw-reading leftover TickFlow, financial last_sync treating leftover parquet mtime as current, minute earliest/latest dates trusting a possibly ungated DuckDB view.
