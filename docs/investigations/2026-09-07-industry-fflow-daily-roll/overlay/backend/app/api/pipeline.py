...
async def run_now(request: Request) -> dict:
    ...
    # 进程内仍在跑的 worker 不是孤儿：只复用，不 fail 后再开第二条 run_now。
        ...
        if existing and existing["status"] == "running":
            ...
            if started:
                try:
                    ...
                    in_flight = bool(daily_pipeline.pipeline_in_flight())
                    if daily_pipeline.should_force_fail_stale_pipeline_job(
                        in_flight=in_flight,
                        elapsed_s=elapsed,
                        stale_after_s=600,
                    ):
                    ...
                    elif elapsed > 600 and in_flight:
                        logger.warning(
                            "job %s still in-flight after %.0fs; reuse without second run_now",
                            existing_id,
                            elapsed,
                        )
                except Exception:
