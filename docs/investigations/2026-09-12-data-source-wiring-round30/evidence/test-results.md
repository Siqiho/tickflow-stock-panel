...
结果：`998 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round30`）。

Round-thirty tests cover leftover TickFlow public/TDX minute skip, leftover TickFlow minute/depth/full-minute/adj skip after custom daily, unreadable leftover calendars fail-closed, custom DuckDB remount empty, HTTP ext instruments_ext gated remount, leftover TickFlow + free realtime `mode=none`, leftover TickFlow daily + public realtime overlay kept, untagged-only leftover still served, after-hours default clock times unchanged.
