...
3. **不可读标记**：daily / minute / quote_snapshot 探测异常一律 fail-closed。
4. **DuckDB**：门控失败只建空视图，不再裸挂 leftover TickFlow / public。
5. **HTTP ext**：`instruments_ext` 走 `re_gate_catalog_views`，不再 leftover-glob。

未改：盘后默认时刻（15:30 / 09:10 / 15:02）、leftover TickFlow 日 K + 公开 realtime 仍 overlay（默认安装，带 `is_quote_snapshot`）、leftover TickFlow 仍可见「只有 untagged」的旧分区、实时 leftover TickFlow + free 仍是 `mode=none`、显式 `adj=public` / `depth5=public`、`.env`、鉴权。
...
云环境结果：`998 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round30`）。详见 `evidence/test-results.md`。
