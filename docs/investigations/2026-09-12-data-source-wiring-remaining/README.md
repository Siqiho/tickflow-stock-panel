...
English summary: remaining silent wrong-source paths after the provider-routing PR. `Cap.QUOTE_POOL` no longer overwrites a custom daily source with TickFlow quotes. Public `pool_provider` is fail-closed (no TickFlow mix on CSI / CN_Equity_A). Live stock/index daily HTTP uses `daily_data_provider`. Custom realtime (`get_realtime` + optional indices) is restored; TickFlow/custom no longer silently top up from public quotes. Pipeline/HTTP minute gates and capability labels honor a configured custom minute source. Entitled TickFlow minute fallback after a custom call failure is unchanged.
...
| 自定义分钟门槛 / 标签 | 管道与能力表只看 TickFlow minute cap | 已配置自定义分钟仍报 public_fallback，全市场同步被跳过 |
...
3. **kline / 分钟门槛**：管道与 HTTP 同步在自定义分钟已解析时放行。已声明源调用失败仍按旧契约回退 TickFlow（需有 minute cap / capset=None）；无资格时不隐式兜底。
...
未改：盘后默认时刻、未声明数据集仍回退 TickFlow 的旧语义、已声明分钟源失败且具备 TickFlow minute cap 时的回退、`.env`、鉴权。TickFlow 默认五档空结果仍可走公开 L1（产品兜底，仅默认 TickFlow）。实时 leftover TickFlow + free 仍是 `mode=none`。本树仍缺 `fetch_intraday_monitor_batch`（`test_custom_provider_indices` 后半段 leftover）。
