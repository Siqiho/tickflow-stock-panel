...
结果：`1093 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round34`）。

Round-thirty-four tests cover catalog / reference leftover-glob no longer mixing untagged extras beside tagged leftover, leftover-part-only hithink / ext snapshot / financial PIT / fund-flow HTTP still serving extras-only leftover, leftover TickFlow watchlist quotes skipping after custom daily, remount leftover-union of unreadable extras staying fail-closed, leftover TickFlow + free realtime `mode=none`, leftover TickFlow daily + public realtime overlay kept, untagged-only leftover still served, after-hours default clock times unchanged. Catalog / get_minute stay fail-loud on leftover TickFlow.
