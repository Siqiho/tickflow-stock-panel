...
结果：`1167 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round38`）。

Round-thirty-eight tests cover leftover TickFlow minute / adj / depth / quote / financial / pool / daily files no longer leftover-serving after custom daily, websocket capability no longer advertising live TickFlow after custom daily, TickFlow provider primitives skipping leftover TickFlow after custom daily, MinuteRefresh leftover TickFlow capability / settings route refresh leftover full-minute loops, catalog leftover TickFlow-routed coverage hiding after custom daily, ALL expansion following leftover daily, leftover TickFlow + free realtime `mode=none`, leftover TickFlow daily + public realtime overlay kept, untagged-only leftover still served, leftover TickFlow single-symbol minute public view kept, after-hours default clock times unchanged. Catalog / get_minute stay fail-loud on leftover TickFlow.
