...
结果：`1129 passed, 47 warnings`（隔离 `DATA_DIR=/tmp/ot-data-source-wiring-round36`）。

Round-thirty-six tests cover FinancialScheduler lifecycle stopping after custom daily, ext remount emptying stale leftover-union views on unreadable / parse failure, snapshot DESCRIBE skipping unreadable extras, share-capital / public financial height staying fail-closed when the gated reader throws, leftover TickFlow index-instrument primitive skipping after custom daily, financial HTTP / capability status no longer advertising live TickFlow after custom daily, sector-monitor stamps skipping unreadable extras, leftover TickFlow + free realtime `mode=none`, leftover TickFlow daily + public realtime overlay kept, untagged-only leftover still served, after-hours default clock times unchanged. Catalog / get_minute stay fail-loud on leftover TickFlow.
