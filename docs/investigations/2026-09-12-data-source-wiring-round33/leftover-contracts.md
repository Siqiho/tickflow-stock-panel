...
Closed in this round (not leftovers): in-memory instrument caches no longer leftover-glob untagged extras; unreadable leftover no longer poisons `scan_usable_daily` / `scan_usable_minute`; leftover-part-only enriched / minute / adj / financial remounts still serve extras-only leftover TickFlow; leftover TickFlow trading-day probe after a custom or unresolved daily; leftover TickFlow corporate-actions `fetch_missing_adj` public sina; unreadable hithink date markers.
