...
export function CompositeStrategyDialog({ open, onClose, onSavedId, editStrategyId }: Props) {
      ...
      api.strategyGet(editStrategyId)
        .then(detail => {
          ...
          if (detail.composite_children) {
            const pcts = toPercentages(detail.composite_children.map((c: { weight: number }) => c.weight))
            setChildren(detail.composite_children.map((c: { strategy_id?: string; id?: string }, i: number) => ({ strategy_id: c.strategy_id ?? c.id ?? '', weight: pcts[i] })))
          }
        })
        .catch(() => {})
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await onSavedId?.(result.strategy_id ?? result.id ?? "")
