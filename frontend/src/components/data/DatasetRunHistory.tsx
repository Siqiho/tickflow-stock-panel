        ...
        {runs.map((run) => (
          <article key={run.run_id} className="rounded-btn border border-border bg-base/40 p-3 text-[11px]">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div><p className="text-xs font-medium text-foreground">{run.operation}</p><p className="mt-0.5 text-muted">发布 {run.rows_published.toLocaleString()} 行</p></div>
            ...
            <AdvancedDisclosure className="mt-2">
              <dl className="grid grid-cols-1 gap-x-4 gap-y-2 rounded-btn bg-base/50 p-3 sm:grid-cols-2">
                <div><dt className="text-muted">run_id</dt><dd className="mt-0.5 break-all font-mono">{run.run_id}</dd></div>
                <div><dt className="text-muted">Provider</dt><dd className="mt-0.5 break-all font-mono">{run.provider ?? '未声明'}</dd></div>
                ...
                {run.error_message && <div className="sm:col-span-2"><dt className="text-muted">错误详情</dt><dd className="mt-0.5 break-words text-danger">{run.error_message}</dd></div>}
              </dl>
            </AdvancedDisclosure>
          </article>
        ))}
