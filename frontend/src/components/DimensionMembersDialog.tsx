...
function useVirtualizer(opts: any) {
  const count = opts?.count ?? 0
  const size = typeof opts?.estimateSize === 'function' ? opts.estimateSize() : 36
  const items = Array.from({ length: count }, (_, index) => ({ index, start: index * size, size, key: index }))
  return {
...
function DimensionMembersDialogContent({ target, onClose, onStockClick }: Omit<Props, 'target'> & { target: DimensionMembersTarget }) {
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    estimateSize: () => 54,
    getItemKey: (index: number) => visibleRows[index]?.symbol ?? index,
    overscan: 8,
  })
