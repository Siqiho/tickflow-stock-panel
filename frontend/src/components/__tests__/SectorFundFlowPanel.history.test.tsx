...
it('opens a complete industry half-year window with a freshness line', async () => {
  vi.mocked(api.fundFlowBoards).mockResolvedValue({
    ok: true,
    items: [
      { code: 'BK0448', name: '通信设备', main_net: 7548600000, change_pct: 0.99, as_of: '2026-08-21 13:57:18' },
  ...
  vi.mocked(api.fundFlowBoardsWindow).mockImplementation(async (days?: number) => {
    if ((days ?? 0) >= 126) {
      return {
        ...
        missing_count: 0,
        coverage_pct: 100,
        items: [{ code: 'BK1033', name: '半年累计第一', main_net: 9_000_000_000, days: 126, window_days: 126 }],
        missing: [],
        prior_available: true,
        source: 'ext_fund_flow_bk_daily',
    ...
    return {
      ...
      covered_count: 128,
      full_count: 128,
      items: [{ code: 'BK0003', name: '五日行业', main_net: 1, days: 5, window_days: 5 }],
      missing: [],
      source: 'ext_fund_flow_bk_daily',
    } as never
  })
  ...
  fireEvent.click(await screen.findByRole('button', { name: /5 天/ }))
  fireEvent.click(screen.getByRole('menuitem', { name: '半年' }))
  expect(await screen.findByText('近126个交易日累计 · 行业日线窗口排名')).toBeInTheDocument()
  expect(await screen.findByText(/满窗 128\/128/)).toBeInTheDocument()
  expect(screen.getByText('数据截止 2026-09-09 · 足够新 · 窗口完整')).toBeInTheDocument()
  expect(screen.getAllByText('半年累计第一').length).toBeGreaterThan(0)
  expect(screen.queryByText('通信设备')).not.toBeInTheDocument()
  expect(vi.mocked(api.fundFlowBoardsWindow).mock.calls.some(call => call[0] === 126)).toBe(true)
})

it('keeps incomplete industry half-year ranks on the daily snapshot', async () => {
  vi.mocked(api.fundFlowBoards).mockResolvedValue({
    ok: true,
    items: [
      { code: 'BK0448', name: '通信设备', main_net: 7548600000, change_pct: 0.99, as_of: '2026-08-21 13:57:18' },
  ...
  vi.mocked(api.fundFlowBoardsWindow).mockResolvedValue({
    ...
    missing_count: 38,
    coverage_pct: 70,
    items: [{ code: 'BK1033', name: '半年残窗', main_net: 1, days: 80, window_days: 126 }],
    missing: [],
    window_note: '窗口数据不足',
    source: 'ext_fund_flow_bk_daily',
  } as never)
  ...
  fireEvent.click(screen.getByRole('menuitem', { name: '半年' }))
  expect(await screen.findByText(/当日快照 · 半年窗口数据不足/)).toBeInTheDocument()
  expect(screen.getByText(/窗口数据不足，当前仍显示当日快照，不能当作半年完整累计榜/)).toBeInTheDocument()
  expect(screen.getAllByText('通信设备').length).toBeGreaterThan(0)
  expect(screen.queryByText('半年残窗')).not.toBeInTheDocument()
})

it('does not request concept 126 or 250 even when those windows would be complete', async () => {
  const conceptWindow = vi.mocked(api.fundFlowConceptsWindow).mockImplementation(async (days?: number) => {
    return {
      ok: true,
      kind: 'concept',
      window_days: days ?? 5,
      requested_days: days ?? 5,
      window_complete: true,
      window_label: `近${days ?? 5}个交易日累计`,
      trading_days: days ?? 5,
      start: '2026-03-11',
      end: '2026-09-09',
      snapshot_count: 504,
      ...
      missing_count: 0,
      coverage_pct: 100,
      items: [{ code: 'BK0986', name: '不该出现的长窗概念', main_net: 9_000_000_000, days: days ?? 5, window_days: days ?? 5 }],
      missing: [],
      prior_available: true,
      source: 'ext_fund_flow_concept_daily',
    } as never
  ...
  expect(screen.getByText(/窗口数据不足，当前仍显示当日快照，不能当作半年完整累计榜/)).toBeInTheDocument()
  expect(screen.getAllByText('通信技术').length).toBeGreaterThan(0)
  expect(screen.queryByText('不该出现的长窗概念')).not.toBeInTheDocument()

  fireEvent.click(await screen.findByRole('button', { name: /半年/ }))
  fireEvent.click(screen.getByRole('menuitem', { name: '1 年' }))
  expect(await screen.findByText(/当日快照 · 1 年窗口数据不足/)).toBeInTheDocument()
  expect(screen.getByText(/窗口数据不足，当前仍显示当日快照，不能当作1 年完整累计榜/)).toBeInTheDocument()
  expect(conceptWindow.mock.calls.every(call => (call[0] ?? 0) <= 63)).toBe(true)
})

it('does not request industry year 250 even when that window would be complete', async () => {
  vi.mocked(api.fundFlowBoards).mockResolvedValue({
    ok: true,
    items: [
      { code: 'BK0448', name: '通信设备', main_net: 7548600000, change_pct: 0.99, as_of: '2026-08-21 13:57:18' },
  ...
  const boardWindow = vi.mocked(api.fundFlowBoardsWindow).mockImplementation(async (days?: number) => {
    return {
      ok: true,
      kind: 'board',
      window_days: days ?? 5,
      ...
      missing_count: 0,
      coverage_pct: 100,
      items: [{ code: 'BK0428', name: '不该出现的年档', main_net: 9_000_000_000, days: days ?? 5, window_days: days ?? 5 }],
      missing: [],
      prior_available: true,
      source: 'ext_fund_flow_bk_daily',
    } as never
  ...
  expect(screen.getByText(/窗口数据不足，当前仍显示当日快照，不能当作1 年完整累计榜/)).toBeInTheDocument()
  expect(screen.getAllByText('通信设备').length).toBeGreaterThan(0)
  expect(screen.queryByText('不该出现的年档')).not.toBeInTheDocument()
  expect(boardWindow.mock.calls.every(call => (call[0] ?? 0) <= 126)).toBe(true)
})
