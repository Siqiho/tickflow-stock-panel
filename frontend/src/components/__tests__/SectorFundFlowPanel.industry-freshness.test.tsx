...
it('shows freshness on an opened industry 126 window', async () => {
  vi.mocked(api.fundFlowBoardsWindow).mockImplementation(async (days?: number) => {
    if ((days ?? 0) >= 126) {
      return {
        ...
        missing_count: 0,
        coverage_pct: 100,
        items: [{ code: 'BK1033', name: '种植业', main_net: 1, days: 126, window_days: 126 }],
        missing: [],
        prior_available: true,
        source: 'ext_fund_flow_bk_daily',
        ...
        freshness_status: 'stale',
        freshness_note: '已陈旧',
        expected_trading_day: '2026-09-11',
        calendar_covers: true,
      } as never
    }
    return {
  ...
  expect(await screen.findByText('数据截止 2026-09-10 · 已陈旧 · 窗口完整')).toBeInTheDocument()
  expect(screen.getByText(/满窗 128\/128/)).toBeInTheDocument()
