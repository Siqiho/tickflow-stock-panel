...
import { buildWatchlistPageContext } from '@/lib/pageContextSnapshots'
...
export function Watchlist() {
  ...
  useEffect(() => {
    setPageContext(buildWatchlistPageContext({
      asOf: enriched.data?.as_of ?? null,
      ...
      viewMode,
      filters: [],
      previewSymbol: previewSymbol ?? undefined,
      previewName: previewName || undefined,
      rows: sortedRows.map((row: any) => ({
        symbol: row.symbol,
        name: row.rt_name ?? row.name,
        change_pct: row.rt_pct ?? row.change_pct,
        rt_pct: row.rt_pct,
        rt_price: row.rt_price,
        close: row.close,
      })),
    }))
    return () => clearPageContext('/watchlist')
  }, [allSymbols.length, enriched.data?.as_of, pendingCount, previewSymbol, sortedRows, viewMode])
