...
export function MiningWorkbench() {
  ...
  const strategyQuery = useQuery({
    queryKey: QK.strategyLinkOptions(),
  ...
  const strategies = strategyQuery.data?.strategies.filter(item =>
    item.execution_backend === 'matrix_native'
    && (item.asset_types ?? ['stock']).includes(draft.assetType)
  ) ?? []
  ...
  }, [strategyQuery.data, draft.assetType])
  ...
  return (
    <div className="grid min-h-[calc(100vh-9rem)] grid-cols-1 overflow-hidden rounded-card border border-border bg-surface xl:grid-cols-[20rem_minmax(0,1fr)]">
      ...
      <section className="min-w-0 bg-surface xl:max-h-[calc(100vh-9rem)] xl:overflow-y-auto">
        ...
        {typeof task.run?.request?.auto_screening === 'object' && task.run.request.auto_screening ? <AutoScreeningCard screening={task.run.request.auto_screening} /> : null}
