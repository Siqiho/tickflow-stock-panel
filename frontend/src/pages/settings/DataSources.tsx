...
import { capabilityLocalFacts } from '@/components/data/ControlPlaneSummary'
import { ExternalReadOnlyData } from '@/components/data/ExternalReadOnlyData'
import { SourceDatasetDetails } from '@/components/data/SourceDatasetDetails'
import {
  buildUnifiedSources,
  sourceById,
...
function CapabilityCard({ cap, pendingKey, onSelect, showRouting = true, readyLabel, localDataLabel }: {
  cap: CapabilityRoute
  ...
  return (
    <article className="rounded-lg border border-border/50 bg-elevated/20 px-3 py-2.5 flex flex-col transition-colors hover:border-border">
      <div className="flex items-center gap-1.5">
        <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-accent/10">
          <Icon className="h-3 w-3 text-accent" />
        </span>
        <h3 className="text-xs font-medium text-foreground truncate">{cap.label}</h3>
      </div>
      ...
      <p className={`mt-1 text-[11px] ${cap.usable ? 'text-foreground' : 'text-warning'}`}>{readyLabel}</p>
      <p className="mt-1 text-[11px] text-secondary">{localDataLabel}</p>

      {/* 候选标签: 管理员点谁该能力就由谁提供; 只读用户只看不改 */}
      <div className="flex flex-wrap gap-1 mt-2 pt-2 border-t border-border/50 min-h-[26px] items-center">
        {cap.candidates.map(c => {
          const active = cap.effective === c.name || cap.current === c.name
          // field=null → 不可路由能力 (仅 TickFlow 提供): 渲染为非交互标签,
          // 保持激活高亮但不可点击 (用 button+disabled 会被 opacity-50 冲淡成灰色)
          const routable = showRouting && cap.field != null
          if (!routable) {
            return (
              <span
                key={c.name}
...
function sourceLocalDatasetCaption(
  catalogState: CatalogReadState,
  datasetCount: number,
  emptyReady: string,
): string {
  if (catalogState === 'loading') return '目录读取中'
  if (catalogState === 'error') return '目录读取失败，暂无法判断'
  return datasetCount > 0 ? `本地 ${datasetCount} 个数据集` : emptyReady
}

/** 能力路由区 (页面主视图): 每个能力一张卡; 管理员点候选即刻切换, 只读用户只看状态 */
  ...
  return (
    <section className="rounded-card border border-border bg-surface p-5">
      ...
      {matrix.isError ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5">
          {list.map(cap => {
            const facts = capabilityLocalFacts(cap, catalog, catalogState)
            return (
              <CapabilityCard
                key={cap.id}
                cap={cap}
                pendingKey={showRouting ? pendingKey : null}
                showRouting={showRouting}
                readyLabel={facts.readyLabel}
                localDataLabel={facts.localDataLabel}
                onSelect={(field, provider) => {
                  if (!showRouting) return
                  routeMut.mutate({ field, provider })
                }}
              />
            )
          })}
...
export function SettingsDataSourcesPanel({
  ...
  const listedNames = new Set(allItems.map(item => item.name))
  const unifiedSources = buildUnifiedSources({
  ...
  const extraSources = unifiedSources.filter(source => !listedNames.has(source.name))
  const selectedUnified = sourceById(unifiedSources, selected)

  const selectedCustom = customList.find(s => s.name === selected)

  // 与总览共用矩阵证据: usable 的生效源才算「服务中」
  const matrixCaps = matrix.data?.capabilities ?? []
  const servingDatasets = (name: string) =>
    matrixCaps.filter((cap) => cap.usable && (cap.effective === name || cap.current === name)).map((cap) => cap.id)
  ...
  return (
    <div id="data-unified-sources" className="space-y-5 max-w-5xl">
      {/* ===== 上区: 能力卡 (管理员可改路由; 只读用户同组卡只看状态) ===== */}
      ...
      <section className="rounded-card border border-border bg-surface p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <h2 className="text-sm font-medium text-foreground">数据来源</h2>
            {!readOnly && (
              <span
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
            ...
            return (
              <div
                ...
                onClick={() => {
                  ...
                  if (!readOnly && customNames.has(item.name)) {
                  ...
                  <span className="text-[10px] text-muted/50 truncate min-w-0">
                    {servingCount > 0
                      ...
                      : pluginUnavailable
                        ...
                        : sourceLocalDatasetCaption(
                          catalogState,
                          sourceById(unifiedSources, item.name)?.datasetIds.length ?? 0,
                          '',
                        )}
                  </span>
                  {!readOnly && (
          ...
          {extraSources.map((source) => {
            const isSelected = selected === source.id || selected === source.name
            const kindLabel = source.kind === 'external'
              ? '外部只读'
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="h-1.5 w-1.5 shrink-0 rounded-full border border-muted/40" />
                  <span className="flex-1 truncate text-sm text-secondary">{source.displayName}</span>
                  <span className="shrink-0 rounded bg-elevated px-1 py-0.5 text-[9px] font-medium leading-none text-secondary">
                    {kindLabel}
                  </span>
                </div>
                <p className="mt-2 text-[10px] text-muted">
                  {sourceLocalDatasetCaption(
                    catalogState,
                    source.datasetIds.length,
                    source.kind === 'external'
                      ? '目前仅两融可查 · 不是全包已入库'
                      : '没有可关联的本地数据集',
                  )}
                </p>
                {source.extraLabels.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {source.extraLabels.map((label) => (
                      <span key={label} className="rounded bg-elevated/60 px-1.5 py-0.5 text-[10px] text-secondary">{label}</span>
                    ))}
                  </div>
                )}
              </button>
        ...
        <div className="mt-3 flex items-center gap-3 text-[10px] text-muted/50 flex-wrap">
          ...
          <span>{readOnly ? '单击卡片查看支持能力、本地数据集、字段与真实生产者' : '单击卡片查看介绍与配置, 点「套用」让该源接管其适配的全部能力'}</span>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {!readOnly && (selected === '__new__' || customNames.has(selected)) && (
            selected === '__new__' ? (
              <DataSourceEditor
                key={selected}
                initial={null}
          ...
          {pluginList.find(x => x.name === selected) && (
            <PluginDetail
              ...
              showKeyConfig={!readOnly}
            />
          )}
          {selectedUnified && selected !== '__new__' && (
            <SourceDatasetDetails
              source={selectedUnified}
              catalog={catalog}
              catalogState={catalogState}
            ...
            >
              {selectedUnified.kind === 'external' && includeExternal ? (
                <div className="mt-3">
                  <ExternalReadOnlyData isAdmin initialSymbol="300502.SZ" />
                </div>
              ) : null}
            </SourceDatasetDetails>
          )}
...
function PluginDetail({ plugin, isActive, matrixCaps, servingSet, showKeyConfig = true }: {
    ...
    <section className="rounded-card border border-border bg-surface p-6">
      ...
      <div className="mt-5 pt-5 border-t border-border grid grid-cols-1 lg:grid-cols-[1fr_1.15fr] gap-6 items-start">
        <div className="min-w-0">
          ...
          {showKeyConfig && plugin.api_key_env && (
...
function TickFlowDetail({ active, matrix, readOnly = false }: { active: boolean; matrix?: CapabilityMatrix; readOnly?: boolean }) {
