...
function AlertExtTags({ ev, fields, onTagClick }: {
  ...
  const conceptTags = getExtTags(ev, fields.concept ?? null)
  const industryTags = getExtTags(ev, fields.industry ?? null)
            ...
            >
              ...
              {r.type === 'sector' ? (
                <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1 pl-0.5">
                  {(r.sector_targets ?? []).slice(0, 3).map((target, idx) => {
                    const item = typeof target === 'string' ? { key: target, name: target } : target
                    return (
                    <span key={item.key ?? item.code ?? item.name ?? idx} className="max-w-28 truncate rounded bg-cyan-500/8 px-1.5 py-0.5 text-[9px] text-cyan-700 dark:text-cyan-300">
                ...
                <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1 pl-0.5">
                  ...
                  <span className="rounded bg-elevated px-1.5 py-0.5 text-[9px] text-secondary">
                    {r.abnormal_window && r.abnormal_window !== 'any' ? `${String(r.abnormal_window).toUpperCase()} 窗口` : '全部窗口'}
                  </span>
                  <span className="rounded bg-elevated px-1.5 py-0.5 text-[9px] text-secondary">
...
function MonitorExtConfigDialog({ open, fields, onClose }: {
  ...
  const qc = useQueryClient()
  const [concept, setConcept] = useState<MonitorExtFieldItem | null>(fields.concept ?? null)
  const [industry, setIndustry] = useState<MonitorExtFieldItem | null>(fields.industry ?? null)
  useEffect(() => { setConcept(fields.concept ?? null); setIndustry(fields.industry ?? null) }, [fields.concept, fields.industry])
