...
_USER_OVERLAY: dict[str, dict[str, FactorSpec]] = {}


def _overlay_user_id() -> str:
    from app.services.user_context import current
    return str(current().get("id") or "owner")


def _is_user_factor(spec: FactorSpec) -> bool:
    return spec.kind in {"custom", "composite"}


def register_factor(spec: FactorSpec) -> None:
    """注册因子; 重复 id 且版本未增时拒绝 (fail-closed)。

    自定义/复合因子进入当前用户 overlay, 不污染其他租户的进程内注册表。
    内置与扩展表因子仍走进程共享 _REGISTRY。
    """
    if _is_user_factor(spec):
        if any(item.id == spec.id for item in _CATALOG):
            raise ValueError(f"不能覆盖内置因子: {spec.id}")
        overlay = _USER_OVERLAY.setdefault(_overlay_user_id(), {})
        existing = overlay.get(spec.id)
        if existing is not None and existing.version >= spec.version:
            raise ValueError(f"factor id 已注册且版本未提升: {spec.id}")
        overlay[spec.id] = spec
        return
...
def get_factor(fid: str) -> FactorSpec | None:
    overlay = _USER_OVERLAY.get(_overlay_user_id(), {})
    if fid in overlay:
        return overlay[fid]
...
def _ordered_specs() -> list[FactorSpec]:
    ...
    overlay = _USER_OVERLAY.get(_overlay_user_id(), {})
    ordered.extend(spec for fid, spec in overlay.items() if fid not in known)
    return ordered
