...
from app.factors.registry import all_factors, get_factor, unregister_factor
...
def list_factors(asset_type: str | None = Query(default=None, pattern="^(stock|etf)$")) -> dict:
    ...
    return {"factors": [_factor_payload(spec) for spec in specs]}


def _factor_payload(spec) -> dict:
    return {
        "id": spec.id,
        "label": spec.label,
        "group": spec.group,
