...
def _path(data_dir: Path | None = None) -> Path:
    ...
    p = user_data_dir(Path(data_dir or settings.data_dir)) / "watchlist.parquet"
...
def _groups_path(data_dir: Path | None = None) -> Path:
    from app.services.user_context import user_data_dir
    p = user_data_dir(Path(data_dir or settings.data_dir)) / "watchlist_groups.json"
    p.parent.mkdir(parents=True, exist_ok=True)
    return p
...
def contains(symbol: str, *, data_dir: Path | None = None) -> bool:
    """Return whether ``symbol`` is in the selected-stock list."""
    p = _path(data_dir)
    if not p.exists():
        return False
    try:
        ...
        logger.warning("watchlist membership read failed: %s", exc)
        return False
    if df.is_empty():
        return False
    wanted = str(symbol or "").strip().upper()
    return bool(df.filter(pl.col("symbol").cast(pl.Utf8).str.to_uppercase() == wanted).height)
