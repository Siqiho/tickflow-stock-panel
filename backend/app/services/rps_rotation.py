...
输出「每列(日期)各自把所有成员按当天涨幅从高到低排序」的矩阵,供前端
「概念/行业分析 → 涨幅RPS轮动」对话框渲染。
...
数据来源全部复用现有资产, 不引入新数据源:
  ...
  - 概念/行业成分股映射: 复用 market_overview_builder 的 _dimension_field / _read_ext_rows /
    _symbol_keys / _dimension_values, 与看板/复盘的维度聚合口径完全一致

性能: 387 概念 / 90 个二级行业 × 30 天的 group_by + sort 是 polars 内存操作, 实测 <50ms;
...
def _load_concept_map_df(repo, kind: str = "concept") -> tuple[pl.DataFrame, int]:
    ...
    if pairs:
        # 去重: 同一 (symbol, 成员) 对会因多 key 形式(SZ/000001)和
...
def build_rps_rotation(
    ...
    if kind != "industry" or level not in (1, 2, 3):
        level = None
