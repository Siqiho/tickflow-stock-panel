...
def _normalize_ratio_columns(df: pl.DataFrame, *names: str) -> pl.DataFrame:
    exprs = []
    for name in names:
        ...
        exprs.append(
            pl.when(col.is_not_null() & (col.abs() > 1.0))
            .then(col / 100.0)
            .otherwise(col)
            .alias(name)
        )
...
def _hist_snapshot(repo: Any) -> dict[str, Any]:
    ...
    if not df.is_empty() and "symbol" in df.columns:
        ...
        df = _normalize_ratio_columns(df.select(cols), "change_pct")
...
def build_intraday(repo: Any, limit: int = 500) -> dict[str, Any]:
    ...
    if df.is_empty() or "symbol" not in df.columns:
        return empty
    df = _normalize_ratio_columns(df, "change_pct", "amplitude")
