...
def worker_principal_from_task(task: dict[str, Any]) -> dict[str, Any] | None:
    """Spawn 子进程不继承 ContextVar; 只接受任务里显式携带的身份。"""
    raw = task.get("principal")
    if not isinstance(raw, dict) or not raw.get("id"):
        return None
    return {
        "id": str(raw["id"]),
        "username": str(raw.get("username") or raw["id"]),
        "role": str(raw.get("role") or "user"),
        "legacy_home": bool(raw.get("legacy_home")),
    }
...
def make_worker_task(kind: str, data_dir: Path, config) -> dict[str, Any]:
    ...
    return {
        ...
        "principal": {
            ...
            "role": ident.get("role"),
            "legacy_home": bool(ident.get("legacy_home")),
        },
    }
...
def _worker_entry(task: dict[str, Any], event_queue, cancel_event) -> None:
    ...
    try:
        ...
        if principal is not None:
            principal_token = user_context.bind(principal)
        ...
        if principal_token is not None:
            with suppress(Exception):
                from app.services import user_context as _user_context
                _user_context.reset(principal_token)
