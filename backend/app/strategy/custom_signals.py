...
def _dir(data_dir: Path) -> Path:
    ...
    d = user_data_dir(data_dir) / "custom_signals"
...
def expression_dependencies(exprs: dict[str, pl.Expr]) -> dict[str, set[str]]:
    """Map compiled signal columns to the root fields they read."""
    return {name: _expr_root_columns(expr) for name, expr in exprs.items()}
