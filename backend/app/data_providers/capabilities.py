...
def _resolve_status(
    ...
    """返回 (effective, current_display, effective_display)。不改 current 原值。"""
    if cap_id == "adj_factor" and raw == _SAME_AS_DAILY:
        daily_raw = current.get("daily_data_provider") or "tickflow"
        daily_effective, _, daily_display = _resolve_status(
            "daily", daily_raw, current, sources,
        )
        return daily_effective, "跟随日K", f"跟随日K · {daily_display}"
    if raw in _PUBLIC_ALIASES.get(cap_id, frozenset()):
        return "public", "公开源", "公开源"
    label = _display_of(sources, raw)
...
def build_capability_matrix(current: dict[str, str], tickflow_tier: str = "none") -> dict:
    ...
    通常即当前值; 仅 same_as_daily / public 别名做状态解析, 不回写偏好。
    ...
    for cap in CAPABILITY_REGISTRY:
        ...
        if cap["id"] in _PUBLIC_CAPABILITIES:
            candidates.append(dict(_PUBLIC_CANDIDATE))
        for s in sources:
        ...
        raw = effective
        resolved, current_display, effective_display = _resolve_status(
            cap["id"], raw, current, sources,
        )
        usable = any(c["name"] == resolved for c in candidates)
        capabilities.append({
            "id": cap["id"],
            "label": cap["label"],
            "desc": cap["desc"],
