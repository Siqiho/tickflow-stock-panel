...
def _seed_hithink_dragon_tiger(data_dir: Path, trade_date: date, *, change: float = 0.05) -> None:
    from app.services.free_sources.hithink_finance import normalize_dragon_tiger

    frame, resolved = normalize_dragon_tiger(
        {
    ...
    dest = (
        data_dir / "reference" / "hithink_dragon_tiger"
        / f"date={resolved.isoformat()}" / "part.parquet"
    )
    dest.parent.mkdir(parents=True, exist_ok=True)
    frame.write_parquet(dest)


def test_hithink_local_fallback_without_fuyao(data_dir, monkeypatch):
    monkeypatch.setattr(dt, "_provider", lambda: None)
    _seed_hithink_dragon_tiger(data_dir, date(2026, 8, 25), change=0.05)
    out = dt.get_dragon_tiger(data_dir, date(2026, 8, 28))
    assert out["state"] == "fallback_prev"
    assert out["source"] == "hithink_official"
    assert out["trade_date"] == "2026-08-25"
    items = out["all"]["stock_items"]
    assert items[0]["thscode"] == "600519.SH"
    assert abs(items[0]["change"] - 0.05) < 1e-9
    assert out["org"]["stock_items"][0]["org_net_value"] == 0.8e8
    assert out["hot_money"]["hot_money_items"] == []
    # 本地 HiThink 不得写入 fuyao JSON 缓存
    assert not (data_dir / "dragon_tiger" / "date=2026-08-25.json").exists()


def test_hithink_local_normalizes_percent_change(data_dir, monkeypatch):
    monkeypatch.setattr(dt, "_provider", lambda: None)
    _seed_hithink_dragon_tiger(data_dir, date(2026, 8, 25), change=5.0)
    out = dt.get_dragon_tiger(data_dir, date(2026, 8, 25))
    assert out["state"] == "ok"
    assert abs(out["all"]["stock_items"][0]["change"] - 0.05) < 1e-9


def test_fuyao_failure_falls_back_to_hithink(data_dir, monkeypatch):
    _use_provider(monkeypatch, _FakeProvider(fail_dates={"2026-08-28", "2026-08-27"}))
    _seed_hithink_dragon_tiger(data_dir, date(2026, 8, 25))
    out = dt.get_dragon_tiger(data_dir, date(2026, 8, 28))
    assert out["state"] == "fallback_prev"
    assert out["source"] == "hithink_official"
    assert out["trade_date"] == "2026-08-25"
