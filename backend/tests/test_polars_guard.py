...
def test_collect_slot_releases_on_exception(monkeypatch) -> None:
    monkeypatch.setattr(guard, "_TOTAL_GATE", threading.BoundedSemaphore(1))
    ...
    thread = threading.Thread(target=_enter)
    thread.start()
    assert entered.wait(timeout=2), "异常后闸位未释放"
    thread.join(timeout=2)


def test_collect_slot_reentry_after_release(monkeypatch) -> None:
    monkeypatch.setattr(guard, "_TOTAL_GATE", threading.BoundedSemaphore(1))
    with collect_slot("interactive"):
        pass
    with collect_slot("interactive"):
        pass
    assert guarded_collect(pl.LazyFrame({"a": [1]}))["a"].to_list() == [1]
