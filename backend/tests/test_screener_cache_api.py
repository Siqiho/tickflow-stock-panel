...
def test_cached_summary_omits_rows_and_counts_realtime_expirations(monkeypatch, tmp_path):
    ...
    # Real cache clock stays on the payload. computed_at is forwarded only
    # when the surviving result item already has it (realtime overlay here does not).
    assert payload["updated_at"] == 1
    assert "computed_at" not in payload["results"]["strategy_a"]


def test_cached_summary_forwards_item_computed_at_without_inventing_cache_time(monkeypatch, tmp_path):
    cached = {
        "as_of": "2026-07-20",
        "results": {
            "strategy_a": {
    ...
    assert payload["results"]["strategy_a"]["computed_at"] == 42
    assert payload["updated_at"] == 99
