...
def test_get_minute_batch_splits_stock_and_etf(monkeypatch):
    ...
    def fake_sync(symbols, *, start_time, end_time, batch_size, rpm, asset_type, **_kw):
        if asset_type == "stock":
...
def _endpoint_mocks(monkeypatch, local_df: pl.DataFrame, sync_ret: pl.DataFrame | None, tmp_path: Path):
    ...
    # 必须用 tmp_path：相对 Path("data") 会落到 cwd/backend/data。
...
def test_minute_batch_tail_stale_pulls_incremental_and_persists(monkeypatch, tmp_path):
    ...
    captured, writes, req = _endpoint_mocks(monkeypatch, local, sync_ret=inc, tmp_path=tmp_path)
...
def test_minute_batch_middle_hole_falls_back_to_full_day(monkeypatch, tmp_path):
    """中间缺K (间距 2min) → 增量窗口永远回看不到洞, 必须退回全天重拉。"""
    from app.api import kline as kline_api

    local = _bars("600519.SH", [datetime(2026, 1, 15, 9, 31), datetime(2026, 1, 15, 9, 33)])
    captured, _, req = _endpoint_mocks(monkeypatch, local, sync_ret=pl.DataFrame(), tmp_path=tmp_path)
...
def test_minute_batch_fresh_local_skips_pull(monkeypatch, tmp_path):
    ...
    captured, writes, req = _endpoint_mocks(monkeypatch, local, sync_ret=pl.DataFrame(), tmp_path=tmp_path)
...
def test_minute_batch_since_returns_only_new_bars(monkeypatch, tmp_path):
    ...
    _, _, req = _endpoint_mocks(monkeypatch, local, sync_ret=pl.DataFrame(), tmp_path=tmp_path)
...
def test_sync_minute_batch_custom_fail_empty_capset_skips_tickflow(monkeypatch):
    """自定义源解析成功但 get_minute 失败时, 空 capset 不得构造/请求 TickFlow。"""
    mock_provider = MagicMock()
...
def test_get_minute_batch_custom_fail_missing_capset_does_not_construct_tickflow(monkeypatch, tmp_path):
    """HTTP 缺 capset + 自定义源解析成功后失败: 仍可走自定义, 但 TickFlow 构造/请求为 0。"""
    from app.api import kline as kline_api
    ...
    get_client = MagicMock(return_value=tickflow)
    monkeypatch.setattr(kline_sync, "get_client", get_client)
    monkeypatch.setattr(kline_api.kline_sync, "get_client", get_client)
    ...
    mock_request = MagicMock()
    mock_request.app.state.repo = mock_repo
    mock_request.app.state.capabilities = None
    ...
    get_client.assert_not_called()
    tickflow.klines.batch.assert_not_called()
    mock_provider.get_minute.assert_called()


def test_get_minute_batch_custom_success_without_tickflow_capability(monkeypatch, tmp_path):
    """普通授权下自定义源成功: 返回数据且 TickFlow 计数为 0。"""
    from app.api import kline as kline_api

    expected = _mock_minute_df("600519.SH")
    mock_provider = MagicMock()
    mock_provider.get_minute.return_value = expected
    _setup_custom_provider(monkeypatch, mock_provider, has_dataset=True)
    ...
    mock_request = MagicMock()
    mock_request.app.state.repo = mock_repo
    mock_request.app.state.capabilities = _empty_capset()
    mock_request.headers = {}
...
def test_sync_minute_batch_none_capset_custom_fail_stays_fail_closed(monkeypatch):
    """已声明自定义分钟源调用失败 fail-closed；capset=None 也不得混 TickFlow。"""
    mock_provider = MagicMock()
    mock_provider.get_minute.side_effect = RuntimeError("custom source unavailable")
    _setup_custom_provider(monkeypatch, mock_provider, has_dataset=True)
    tickflow = MagicMock()
    tickflow.klines.batch.side_effect = Exception("tickflow fail")
    get_client = MagicMock(return_value=tickflow)
    monkeypatch.setattr(kline_sync, "get_client", get_client)
...
def test_http_minute_batch_missing_capset_does_not_pull(monkeypatch):
    """HTTP 入口缺少 capabilities 时 fail-closed: 只读本地, 不调用 TickFlow 补拉。"""
    from app.api import kline as kline_api

    sync_spy = MagicMock(side_effect=AssertionError("must not pull without capset"))
    monkeypatch.setattr(kline_api.kline_sync, "sync_minute_batch", sync_spy)
    get_client = MagicMock(side_effect=AssertionError("must not call TickFlow"))
    monkeypatch.setattr(kline_api.kline_sync, "get_client", get_client)
...
def test_gzip_q0_is_not_compressed(monkeypatch):
    """Accept-Encoding: gzip;q=0 明确拒绝压缩, 即使开关开也返回明文 JSON。"""
    from app.api import kline as kline_api
    from fastapi import Response

    mock_request, _ = _compress_mock_env(monkeypatch, compress_on=True, accept="gzip;q=0")
    result = kline_api.get_minute_batch(
        mock_request, {"symbols": ["600519.SH"], "date": "2026-01-15"}
    )
...
def test_gzip_response_keeps_json_and_vary(monkeypatch):
    """正常 gzip 必须可解压回完整 JSON, 并带 Vary: Accept-Encoding。"""
    import gzip as gzip_mod
    import json as json_mod
    from app.api import kline as kline_api
    from fastapi import Response

    mock_request, _ = _compress_mock_env(monkeypatch, compress_on=True, accept="gzip")
    result = kline_api.get_minute_batch(
        mock_request, {"symbols": ["600519.SH"], "date": "2026-01-15"}
    )
    assert isinstance(result, Response)
    ...
    payload = json_mod.loads(gzip_mod.decompress(result.body))
    assert "600519.SH" in payload["data"]
    assert payload["full_minute_local"] is False
