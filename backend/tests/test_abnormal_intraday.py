...
from app.services.abnormal_moves import build_intraday, build_overview, _hist_cache, _hist_cache_lock
...
def test_intraday_normalizes_percent_change_pct():
    repo = _FakeRepo(_df([
        {"symbol": "301151.SZ", "name": "创业板涨停", "close": 20.74,
         "change_pct": 20.023148, "amplitude": 21.5, "signal_limit_up": True},
        {"symbol": "600000.SH", "name": "已是小数", "close": 10.2,
         "change_pct": 0.02, "amplitude": 0.03, "signal_volume_surge": True},
    ]))
    out = build_intraday(repo)
    by_sym = {row["symbol"]: row for row in out["rows"]}
    assert abs(by_sym["301151.SZ"]["change_pct"] - 0.20023148) < 1e-9
    assert abs(by_sym["301151.SZ"]["amplitude"] - 0.215) < 1e-9
    assert abs(by_sym["600000.SH"]["change_pct"] - 0.02) < 1e-9
    assert abs(by_sym["600000.SH"]["amplitude"] - 0.03) < 1e-9


def test_overview_normalizes_percent_rt_pct():
    ...
    repo.get_enriched_latest = lambda: (repo._df, date.today())  # type: ignore[method-assign]
    out = build_overview(repo, None, min_closeness=0.5)
    assert abs(out["rows"][0]["rt_pct"] - 0.20023148) < 1e-9
