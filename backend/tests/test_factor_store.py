...
def test_custom_factors_are_isolated_across_users(tmp_path) -> None:
    from app.services import user_context

    definition = {
        "id": "uf_alice_only", "kind": "custom", "version": 1, "label": "Alice",
        "formula": "close + 1", "status": "draft",
    }
    ...
    try:
        store.save_one(tmp_path, definition)
        assert [item["id"] for item in store.load_all(tmp_path)] == ["uf_alice_only"]
        alice_path = tmp_path / "tenants" / "alice" / "user_data" / "custom_factors" / "uf_alice_only.json"
    ...
    try:
        ...
        store.save_one(tmp_path, {**definition, "id": "uf_bob_only", "label": "Bob"})
        assert [item["id"] for item in store.load_all(tmp_path)] == ["uf_bob_only"]
        assert not (tmp_path / "tenants" / "bob" / "user_data" / "custom_factors" / "uf_alice_only.json").exists()
    finally:
        user_context.reset(token)
