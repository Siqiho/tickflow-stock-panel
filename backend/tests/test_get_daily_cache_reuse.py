...
def test_get_daily_columns_fast_path_skips_warmup_compute():
    raw = _raw_frame()
    dates = raw["date"].to_list()
    repo, calls = _bare_repo(raw)
    compute_calls = {"n": 0}
    real_compute = repo._compute_enriched_range

    def spy(df):
        compute_calls["n"] += 1
        return real_compute(df)

    repo._compute_enriched_range = spy  # type: ignore[method-assign]
    cols = ["symbol", "date", "open", "high", "low", "close"]
    out = repo.get_daily(SYM, dates[0], dates[-1], columns=cols)
