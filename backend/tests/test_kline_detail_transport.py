...
def _isolated_settings_and_clock(monkeypatch):
    ...
    # 本地公开分时兜底与 9a4「无权限=空」断言分开: 本文件只验 TickFlow 门控
    monkeypatch.setattr(
        "app.services.kline_sync._public_minute_fallback",
...
class _DetailRepo:
    ...
    def get_daily(self, symbol, start, end, columns=None):
        return self.get_daily_asset("stock", symbol, start, end, columns=columns)
