...
def test_lots_are_isolated_across_users(tmp_path):
    from app.services import user_context

    alice = {"id": "alice", "username": "alice", "role": "user"}
    bob = {"id": "bob", "username": "bob", "role": "user"}
    lot = lots_domain.normalize_lot(_lot(id="lot_alice1"))
    token = user_context.bind(alice)
    try:
        lots_domain.save_one(tmp_path, lot)
        assert lots_domain.load_all(tmp_path)[0]["id"] == "lot_alice1"
        alice_path = tmp_path / "tenants" / "alice" / "user_data" / "lots" / "lot_alice1.json"
    ...
    try:
        assert lots_domain.load_all(tmp_path) == []
        lots_domain.save_one(tmp_path, lots_domain.normalize_lot(_lot(id="lot_bob1")))
        ids = {item["id"] for item in lots_domain.load_all(tmp_path)}
        assert ids == {"lot_bob1"}
        assert not (tmp_path / "tenants" / "bob" / "user_data" / "lots" / "lot_alice1.json").exists()
    finally:
        user_context.reset(token)
